...
public class SearchService {
    ...
    public Optional<Candy> search(String candyName) {
        Optional<Candy> candyinwarehouse = warehouse.search(candyName);
        if (candyinwarehouse.isPresent()) {
            return candyinwarehouse;
        } else {
    ...
    private Optional<Candy> supplierSearch(String candyName) {
        ...
        return srm.listSuppliers().stream()
                .filter(supplierName -> srm.getProduct(supplierName, candyName) != null)
